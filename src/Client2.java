...
public class Client2 {
	...
	public void socket_clnt() throws IOException, InterruptedException
    {
		...
		try{
			...
			System.out.println("Client2 could not connect to the server...");
		}
		System.out.println("Client2 connected to the Main Server for receiving it's set of chunks at " + socket.getInetAddress().getHostAddress()+":"+SPORT);
		PrintWriter printWriter = new PrintWriter(socket.getOutputStream(),true);
		//while(true){
		//inform the client about the client number
		String readerInput = "2";
		BufferedInputStream bis = new BufferedInputStream(socket.getInputStream());
		DataInputStream dis = new DataInputStream(bis);
		printWriter.println(readerInput);
		String dirPath = "C:/output2/", full_filename= null;
        ...
        ClientClient2 CC = new ClientClient2(anArray, arrlnth);
        CC.start();
        ClientServer2 CS = new ClientServer2(anArray, arrlnth);
        CS.start();

    }
}
	
class ClientClient2 extends Thread
{
    int dwldScktNos;
    private int[] anArray;
    private int arrlnth;
    ...
    public ClientClient2(int[] anArray, int arrlnth)
    {
        ...
        try
        {
            ...
            while (status_chk) {
                try {
					...
					status_chk = false;
					System.out.println ("Client2 connected to it's Download Neigbour: 1");
					// Write Logic for file Download from Client1
                    ...
                    while(file_complete){
                    	if(i==arrlnth){
                    		...
                    		if (chnk_presence==1){
                    			...
                    			anArray[i]=1;
                    			i= i+1;
                    			System.out.println("Chunk "+i+" Arrived from Client1");
                    		}
                    		else{
                    			i=i+1;
                    			continue;
    						...
    						if (files2.length == arrlnth) {
    							...
    							String sourcefilepath = "C:/output2/";
...
class ClientServer2 extends Thread
{
    String line;
    int upldCl;
    int upldScktNos;
    ...
    public ClientServer2(int[] anArray, int arrlnth)
    {
    	this.anArray = anArray;
    	this.arrlnth = arrlnth;
    }
    ...
    public void run()
    {
        ...
        {	
            ...
            System.out.println("Client2 connected to: " + upldSckt.getInetAddress().getHostAddress()+":"+UPORT+" as Server now for Client3");
				...
				if (chnk_val == 0) {
					...
					for (File file : files) {
						...
						if (name.indexOf(String.valueOf(formatted)) > -1) {
							...
							bos.flush();
							bis.close();
							System.out.println("Chunk "+(chnk_no+1)+" Sent to the Client3");
							break;
						} 

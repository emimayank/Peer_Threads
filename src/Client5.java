...
public class Client5 {
	...
	public void socket_clnt() throws IOException, InterruptedException {
		...
		try{
			...
			System.out.println("Client5 could not connect to the server...");
		}
		System.out.println("Client5 connected to the Main Server for receiving it's set of chunks at " + socket.getInetAddress().getHostAddress()+":"+SPORT);
		PrintWriter printWriter = new PrintWriter(socket.getOutputStream(), true);
		// while(true){
		// inform the client about the client number
		String readerInput = "5";
		BufferedInputStream bis = new BufferedInputStream(socket.getInputStream());
		DataInputStream dis = new DataInputStream(bis);
		printWriter.println(readerInput);
		String dirPath = "C:/output5/", full_filename=null;
		...
		ClientClient5 CC = new ClientClient5(anArray, arrlnth);
		CC.start();
		ClientServer5 CS = new ClientServer5(anArray, arrlnth);
		CS.start();

	}
}

class ClientClient5 extends Thread {
	int dwldScktNos;
	private int[] anArray;
	private int arrlnth;
	Socket dwldSckt;

	public ClientClient5(int[] anArray, int arrlnth) {
		...
		try {
			...
			while (status_chk) {
				try {
					...
					status_chk = false;
					System.out.println ("Client5 connected to it's Download Neigbour: 4");
					...
					while (file_complete) {
						...
						if (chnk_presence == 1) {
							...
							anArray[i] = 1;
							i = i + 1;
							System.out.println("Chunk "+i+" Arrived from Client4");
						} else {
							i = i + 1;
							continue;
						}
						String directory = "C:/output5/";
						...
						if (files2.length == arrlnth) {
							...
							String sourcefilepath = "C:/output5/";
...
class ClientServer5 extends Thread {
	String line;
	int upldCl;
	int upldScktNos;
	private int[] anArray;
	...
	public ClientServer5(int[] anArray, int arrlnth) {
		this.anArray = anArray;
		this.arrlnth = arrlnth;
	}
	...
	public void run() {
		try {
			...
			System.out.println("Client5 connected to: " + upldSckt.getInetAddress().getHostAddress()+":"+UPORT+" as Server now for Client1");
				...
				if (chnk_val == 0) {
					...
					for (File file : files) {
						...
						if (name.indexOf(String.valueOf(formatted)) > -1) {
							...
							bos.flush();
							bis.close();
							System.out.println("Chunk "+(chnk_no+1)+" Sent to the Client1");
							break;
						}

...
public class Client3 {
	...
	public void socket_clnt() throws IOException, InterruptedException {
		...
		try {
			...
			System.out.println("Client3 could not connect to the server...");
		}
		System.out.println("Client3 connected to the Main Server for receiving it's set of chunks at " + socket.getInetAddress().getHostAddress()+":"+SPORT);
		PrintWriter printWriter = new PrintWriter(socket.getOutputStream(), true);
		// while(true){
		// inform the client about the client number
		String readerInput = "3";
		BufferedInputStream bis = new BufferedInputStream(socket.getInputStream());
		DataInputStream dis = new DataInputStream(bis);
		printWriter.println(readerInput);
		String dirPath = "C:/output3/", full_filename=null;
		...
		ClientClient3 CC = new ClientClient3(anArray, arrlnth);
		CC.start();
		ClientServer3 CS = new ClientServer3(anArray, arrlnth);
		CS.start();

	}
}

class ClientClient3 extends Thread {
	int dwldScktNos;
	private int[] anArray;
	private int arrlnth;
	Socket dwldSckt;

	public ClientClient3(int[] anArray, int arrlnth) {
		...
		try {
			...
			while (status_chk) {
				try {
					...
					status_chk = false;
					System.out.println("Client3 connected to it's Download Neigbour: 2");
					...
					while (file_complete) {
						...
						if (chnk_presence == 1) {
							...
							anArray[i] = 1;
							i = i + 1;
							System.out.println("Chunk "+i+" Arrived from Client2");
						} else {
							i = i + 1;
							continue;
						}
                		String directory = "C:/output3/";
						...
						if (files2.length == arrlnth) {
							...
							String sourcefilepath = "C:/output3/";
...
class ClientServer3 extends Thread {
	String line;
	int upldCl;
	int upldScktNos;
	private int[] anArray;
	...
	public ClientServer3(int[] anArray, int arrlnth) {
		this.anArray = anArray;
		this.arrlnth = arrlnth;
	}
	...
	public void run() {
		try {
			...
			System.out.println("Server1 connected to: " + upldSckt.getInetAddress().getHostAddress());
				...
				if (chnk_val == 0) {
					...
					for (File file : files) {
						...
						if (name.indexOf(String.valueOf(formatted)) > -1) {
							...
							bos.flush();
							bis.close();
							System.out.println("Client3 connected to: " + upldSckt.getInetAddress().getHostAddress()+":"+UPORT+" as Server now for Client4");
							break;
						}

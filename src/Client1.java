...
public class Client1 {
	...
	public void socket_clnt() throws IOException, InterruptedException {
		...
		try {
			...
			System.out.println("Client1 could not connect to the server...");
		}
		System.out.println("Client1 connected to the Main Server for receiving it's set of chunks at "
				+ socket.getInetAddress().getHostAddress() + ":" + SPORT);
		PrintWriter printWriter = new PrintWriter(socket.getOutputStream(), true);
		// while(true){
		// inform the client about the client number
		String readerInput = "1";
		BufferedInputStream bis = new BufferedInputStream(socket.getInputStream());
		DataInputStream dis = new DataInputStream(bis);
		printWriter.println(readerInput);
		String dirPath = "C:/output1/", full_filename = null;
		...
		ClientClient1 CC = new ClientClient1(anArray, arrlnth);
		CC.start();
		ClientServer1 CS = new ClientServer1(anArray, arrlnth);
		CS.start();

	}
}

class ClientClient1 extends Thread {
	int dwldScktNos;
	private int[] anArray;
	private int arrlnth;
	Socket dwldSckt;

	public ClientClient1(int[] anArray, int arrlnth) {
				...
				try {
					...
					while ((tempStr = read.readLine()) != null) {
						...
						if (tempStr.charAt(0) == '6') {
							...
							// return Integer.parseInt(tempStr.substring(7,
							// 11));
						}
					}
					read.close();
					...
					status_chk = false;
					System.out.println("Client1 connected to it's Download Neigbour: 5");
					...
					while (file_complete) {
						...
						if (chnk_presence == 1) {
							...
							anArray[i] = 1;
							i = i + 1;
							System.out.println("Chunk " + i + " Arrived from Client5");
						} else {
							i = i + 1;
							continue;
						}

						String directory = "C:/output1/";
						...
						if (files2.length == arrlnth) {
							...
							String sourcefilepath = "C:/output1/";
...
class ClientServer1 extends Thread {
	String line;
	int upldCl;
	int upldScktNos;
	private int[] anArray;
	...
	public ClientServer1(int[] anArray, int arrlnth) {
		this.anArray = anArray;
		this.arrlnth = arrlnth;
	}
	...
	public void run() {
		try {
			...
			System.out.println("Client1 connected to: " + upldSckt.getInetAddress().getHostAddress() + ":" + UPORT
					+ " as Server now for Client2");
			// Write Logic for File Upload for Client1
			boolean file_complete = true;
			String message = null;
				...
				if (chnk_val == 0) {
					...
					for (File file : files) {
						...
						if (name.indexOf(String.valueOf(formatted)) > -1) {
							...
							bos.flush();
							bis.close();
							System.out.println("Chunk " + (chnk_no + 1) + " Sent to the Client2");
							break;
						}
